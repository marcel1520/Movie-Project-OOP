...
class StorageJson(IStorage):
    ...
    def fetch_movie_data(self, title):
        ...
        try:
            req = requests.get(url, timeout=5)  # stops after 5 seconds if connection cannot be established
            movie_data = req.json()
    ...
    def load_movies(self):
        with open(self.file_path, "r") as file:
            conv_movie_file = json.load(file)
            return conv_movie_file
    ...
    def list_movies(self):
        ...
        for movie_info in self.movies.values():
            all_films += (f"{movie_info['title']} -- "
                          ...
                          f"runtime: {movie_info['runtime']}\n")

        return all_films

    def save_movie(self):
        formatted_file = json.dumps(self.movies, indent=4)
    ...
    def update_movie(self, title, rating):
        ...
        if update_title in self.movies:
            try:
                ...
                update_runtime = movie_info["Runtime"]
